...
shrink_fup_stan = "
  ...
  model {
    ...
    target += -10*theta + 0.5*(log(-200*theta^2-20*theta-1+exp(20*theta))-log(theta^3));
...
writeLines(shrink_fup_stan, "random_shrink_fup.stan")
...
simulate_theta_posterior_mixed <- function(nreps = 200, 
  ...
  for (repl in 1:nreps) {
    ...
    # generate a dataset
    stan_data_list <- generate_dataset(NumReps = 7, J = nsubjects, true_theta=true_theta, sigma_b, sigma_eps)
    data_storage[,repl] <- stan_data_list$y
      
    # get the posterior sample for theta
    # Compile and fit the Stan model
    fit <- stan(
      file = "random_shrink_fup.stan",
      data = stan_data_list,
  ...
  filename_data =   paste(format(Sys.time(), "%b-%d-%Hh%M"),"-mixed-effects-shrink-data-FUP-",nsubjects,".csv",sep="")
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-mixed-effects-shrink-posterior-sample-FUP-",nsubjects,".csv",sep="")
...
thetas_n15 <- simulate_theta_posterior_mixed(nreps=200,nsubjects = 15, M=5000, sigma_b=0.3, sigma_eps = 0.03)
thetas_n50 <- simulate_theta_posterior_mixed(nreps=200,nsubjects = 50, M=5000, sigma_b=0.3, sigma_eps = 0.03)
thetas_n100 <- simulate_theta_posterior_mixed(nreps=200,nsubjects = 100, M=5000, sigma_b=0.3, sigma_eps = 0.03)
