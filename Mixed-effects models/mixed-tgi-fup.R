...
tgi_model_random_effects = "
  ...
  parameters {
    ...
    real<lower=0.01,upper=4> theta_s; // decay parameter 
  ...
  model {
    ...
    target += -4*theta_s + (log(-8*theta_s^2-4*theta_s-1+exp(4*theta_s))-log(4*theta_s^3));
    
    // Custom log prior for theta_g (as provided)
    target += 4*theta_g + (log(8*theta_g^2-4*theta_g+1-exp(-4*theta_g))-log(4*theta_g^3));
...
writeLines(tgi_model_random_effects, "tgi_mixed_model.stan")

generate_tgi_dataset <- function(NumReps = 8, J, 
                                 true_theta_g, sigma_b_g,
                                 true_theta_s, sigma_b_s, sigma_eps) {
...
simulate_tgi_posterior_mixed <- function(nreps, nsubjects, 
  ...
  for (repl in 1:nreps) {
    ...
    init_fun <- function() {
      list(
        theta_g = 0.5,  # within your bounds [0.001, 2.7]
        theta_s = 1.5   # within your bounds [0.001, 7.5]
      )
    }
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-DataV2-mixed-effects-growth-posterior-sample-FUP-",nsubjects,".csv",sep="")
