...
tgi_unif_model_random_effects = "
  ...
  parameters {
    real<lower=0.01,upper=2.7> theta_g; // growth parameter
    real<lower=1,upper=7> theta_s; // decay parameter 
...
generate_tgi_dataset <- function(NumReps = 8, J, 
                                 true_theta_g = exp(-0.2), sigma_b_g=0.1,
...
simulate_tgi_posterior_mixed <- function(nreps, nsubjects, 
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-DataV2-mixed-effects-growth-posterior-sample-uniform-MEDIUM-",nsubjects,".csv",sep="")
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-DataV2-mixed-effects-shrink-posterior-sample-uniform-MEDIUM-",nsubjects,".csv",sep="")
...
par(mfrow=c(1,1))
# 
# n15_theta_s_samples <- as.matrix(n15_samples_list[["theta_s_MH"]])
# n15_theta_g_samples <- as.matrix(n15_samples_list[["theta_g_MH"]])

theta_g_unif = read.csv("Aug-07-01h55-mixed-effects-growth-posterior-sample-uniform50.csv", header = TRUE)
theta_s_unif =read.csv("Aug-07-01h55-mixed-effects-shrink-posterior-sample-uniform50.csv", header = TRUE)

# n50_theta_s_samples <- as.matrix(thetas_n50[["theta_s"]])
# n50_theta_g_samples <- as.matrix(thetas_n50[["theta_g"]])

n50_theta_s_samples = as.matrix(theta_s_unif)
n50_theta_g_samples = as.matrix(theta_g_unif)

colors <- rainbow(10)
plot(n50_theta_s_samples[,1], type ="l", col = colors[1], ylim = c(0,3))
for (i in 2:ncol(n50_theta_s_samples)) {
  if (thetas_n50$rhats[i,1]>1.1) {
    lines(n50_theta_s_samples[,i], col = colors[i])
  }
}

# n100_theta_s_samples <- as.matrix(n100_samples_list[["theta_s_MH"]])
# n100_theta_g_samples <- as.matrix(n100_samples_list[["theta_g_MH"]])
...
# stats15_s = statistics_calc(exp(0.83), n15_theta_s_samples, FALSE)
stats50_s = statistics_calc(exp(0.83), n50_theta_s_samples, FALSE)
# stats100_s = statistics_calc(exp(0.83), n100_theta_s_samples, FALSE)

# tgi_shrink_fup_statistics_table = rbind(stats15_s, stats50_s, stats100_s)
# row.names(tgi_shrink_fup_statistics_table) = c("n=15", "n=50", "n=100")
# colnames(tgi_shrink_fup_statistics_table) = c("CP_90", "CP_95", "ILE90", "ILE95", 
#                                               "MAE_mean", "MAE_median")
# round(tgi_shrink_fup_statistics_table,4)

# growth table
# stats15_g = statistics_calc(theta_g_true, n15_theta_g_samples, FALSE)
stats50_g = statistics_calc(exp(-0.2), n50_theta_g_samples, FALSE)
# stats100_g = statistics_calc(theta_g_true, n100_theta_g_samples, FALSE)

# tgi_growth_fup_statistics_table = rbind(stats15_g, stats50_g, stats100_g)
# row.names(tgi_growth_fup_statistics_table) = c("n=15", "n=50", "n=100")
# colnames(tgi_growth_fup_statistics_table) = c("CP_90", "CP_95", "ILE90", "ILE95", 
#                                               "MAE_mean", "MAE_median")
# round(tgi_growth_fup_statistics_table,4)
...
# Assuming your datasets are loaded as data frames
g_chain15 <- n15_theta_g_samples[, 1]  # First column = posterior samples
g_chain50 <- n50_theta_g_samples[, 1]  # First column = posterior samples
g_chain100 <- n100_theta_g_samples[, 1]  # First column = posterior samples

s_chain15 <- n15_theta_s_samples[, 1]  # First column = posterior samples
s_chain50 <- n50_theta_s_samples[, 1]  # First column = posterior samples
s_chain100 <- n100_theta_s_samples[, 1]  # First column = posterior samples

par(mfrow = c(2, 3))  # Side-by-side plots

acf(g_chain15, lag.max = 20, main = "ACF - growth n=15", 
    ylab = "Autocorrelation", xlab = "Lag")
acf(g_chain50, lag.max = 20, main = "ACF - growth n=50", 
    ylab = "Autocorrelation", xlab = "Lag")
acf(g_chain100, lag.max = 20, main = "ACF - growth n=100", 
    ylab = "Autocorrelation", xlab = "Lag")

acf(s_chain15, lag.max = 20, main = "ACF - shrinkage n=15", 
    ylab = "Autocorrelation", xlab = "Lag")
acf(s_chain50, lag.max = 20, main = "ACF - shrinkage n=50", 
    ylab = "Autocorrelation", xlab = "Lag")
acf(s_chain100, lag.max = 20, main = "ACF - shrinkage n=100", 
    ylab = "Autocorrelation", xlab = "Lag")
