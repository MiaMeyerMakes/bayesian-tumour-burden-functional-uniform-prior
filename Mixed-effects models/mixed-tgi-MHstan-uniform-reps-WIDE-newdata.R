...
tgi_unif_model_random_effects = "
  ...
  parameters {
    ...
    real<lower=0.1,upper=10> theta_s; // decay parameter 
...
simulate_tgi_posterior_mixed <- function(nreps, nsubjects, 
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-DataV2-mixed-effects-growth-posterior-sample-uniform-WIDE-",nsubjects,".csv",sep="")
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-DataV2-mixed-effects-shrink-posterior-sample-uniform-WIDE-",nsubjects,".csv",sep="")
