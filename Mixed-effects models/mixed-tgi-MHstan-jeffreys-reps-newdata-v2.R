...
tgi_jef_model_random_effects = "
  ...
  parameters {
    real<lower=0.01,upper=2.5> theta_g; // growth parameter
    real<lower=0.135,upper=7.5> theta_s; // decay parameter 
  ...
  model {
    ...
    {
      real fisher_s_info_sum = 0;
      real fisher_g_info_sum = 0;
      
      // Sum over all subjects i
      for (j in 1:J) {
        real subject_s_sum = 0;
        real subject_g_sum = 0;
        
        // Sum over all time points j for subject i
        for (n in 1:N) {
          if (group[n] == j) {  // if observation n belongs to subject i
            real t_ij = x[n];   // time point
            real exp_bsi = exp(b_s[j]);
            real exp_s_term = exp(-2 * theta_s * t_ij * exp_bsi);
            
            real exp_bgi = exp(b_g[j]);
            real exp_g_term = exp(2 * theta_g * t_ij * exp_bgi);
            
            subject_s_sum += square(t_ij) * square(exp_bsi) * exp_s_term;
            subject_g_sum += square(t_ij) * square(exp_bgi) * exp_g_term;
          }
        }
        
        fisher_s_info_sum += subject_s_sum;
        fisher_g_info_sum += subject_g_sum;
      }
      
      // Add log of Jeffreys prior: log(sqrt(fisher_s_info_sum)) = 0.5 * log(fisher_s_info_sum)
      if (fisher_s_info_sum > 0) {
        target += 0.5 * log(fisher_s_info_sum);
      }
      if (fisher_g_info_sum > 0) {
        target += 0.5 * log(fisher_g_info_sum);
      }
    }
...
writeLines(tgi_jef_model_random_effects, "tgi_mixed_model_jef.stan")
...
simulate_tgi_jef_posterior_mixed <- function(nreps = 1, nsubjects, 
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-DataV2-mixed-effects-growth-posterior-sample-jeffreys",nsubjects,".csv",sep="")
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-DataV2-mixed-effects-shrink-posterior-sample-jeffreys",nsubjects,".csv",sep="")
...
thetas_n15_jef <- simulate_tgi_jef_posterior_mixed(nreps=250,nsubjects = 15,
                                                   true_theta_g = exp(-2), sigma_b_g=0.1,
                                                   true_theta_s =  exp(0.83), sigma_b_s=0.7, 
                                                   sigma_eps=0.05, M = 7000, nchains =1)
thetas_n50_jef <- simulate_tgi_jef_posterior_mixed(nreps = 250, nsubjects=50, 
                                                   true_theta_g = exp(-2), sigma_b_g=0.1,
                                                   true_theta_s =  exp(0.83), sigma_b_s=0.7, 
                                                   sigma_eps=0.05, M = 7000, nchains =1)
thetas_n100_jef <- simulate_tgi_jef_posterior_mixed(nreps=250,nsubjects = 100, 
                                                    true_theta_g = exp(-2), sigma_b_g=0.1,
                                                    true_theta_s =  exp(0.83), sigma_b_s=0.7, 
                                                    sigma_eps=0.05, M = 7000, nchains =1)
