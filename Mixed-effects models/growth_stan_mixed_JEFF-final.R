...
setwd("~/Desktop/Thesis/code/growth model/mixed-effects")
...
growth_jeff_stan = "
  ...
  parameters {
    real<lower=0,upper=2> theta; // growth parameter (with your bounds)
...
writeLines(growth_jeff_stan, "random_growth_jeff.stan")
...
simulate_theta_posterior_mixed <- function(nreps = 200, 
  ...
  filename_data =   paste(format(Sys.time(), "%b-%d-%Hh%M"),"-mixed-effects-growth-data-JEFF-",nsubjects,".csv",sep="")
  ...
  filename = paste(format(Sys.time(), "%b-%d-%Hh%M"),"-mixed-effects-growth-posterior-sample-JEFF-",nsubjects,".csv",sep="")
