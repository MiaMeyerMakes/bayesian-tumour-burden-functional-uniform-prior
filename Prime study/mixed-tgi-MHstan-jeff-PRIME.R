...
writeLines(tgi_prime_jeff_model, "tgi_prime_jeff_model.stan")
...
fit_jeff <- stan(
  file = "tgi_prime_jeff_model.stan",
  data = stan_prime_dat,
...
rstan::traceplot(fit_jeff, pars = c("theta_g", "theta_s"))

stan_jeff_summary <- summary(fit_jeff)$summary

jeff_posterior_samples <- rstan::extract(fit_jeff, permuted = TRUE)

theta_rhat <- stan_jeff_summary[c("theta_g","theta_s"), "Rhat"]
theta_pointest <- stan_jeff_summary[c("theta_g","theta_s"), "mean"]
print(theta_pointest)

biom.datf <- as.data.frame(biom.df.filtered)
biom.datf$pred_jeff_stan <- colMeans(jeff_posterior_samples$y_pred)
...
comparison_plot <- ggplot(prime_subset, aes(x = BIOMYR)) +
  ...
  geom_point(aes(y = pred_jeff_stan, color = "Bayes"), size = 1) +
  geom_line(aes(y = pred_jeff_stan, color = "Bayes"), linetype = "dashed", linewidth = 0.6) +
  facet_wrap(~ SUBJID, scales = "free_y") +
  labs(
    title = "Bayesian TGI Model Fit with Jeffreys prior on Prime Data",
    y = "SLD",
    x = "Time (years)",
    color = "Type"
  ) +
