...
tgi_prime_fup_model = "
  ...
  model {
    ...
    // Custom log prior for theta_s (as provided)
    target += -6*theta_s + (log(-18*theta_s^2-6*theta_s-1+exp(6*theta_s))-log(4*theta_s^3));
    
    // Custom log prior for theta_g (as provided)
    target += 6*theta_g + (log(18*theta_g^2-6*theta_g+1-exp(-6*theta_g))-log(4*theta_g^3));
...
writeLines(tgi_prime_fup_model, "tgi_prime_fup_model.stan")
...
rstan::traceplot(fit_fup, pars = c("theta_g", "theta_s"))

stan_fup_summary <- summary(fit_fup)$summary

fup_posterior_samples <- rstan::extract(fit_fup, permuted = TRUE)

theta_rhat <- stan_fup_summary[c("theta_g","theta_s"), "Rhat"]
theta_pointest <- stan_fup_summary[c("theta_g","theta_s"), "mean"]
print(theta_pointest)

biom.datf <- as.data.frame(biom.df.filtered)
biom.datf$pred_fup_stan <- colMeans(fup_posterior_samples$y_pred)
...
comparison_plot <- ggplot(prime_subset, aes(x = BIOMYR)) +
  ...
  geom_point(aes(y = pred_fup_stan, color = "Bayes"), size = 1) +
  geom_line(aes(y = pred_fup_stan, color = "Bayes"), linetype = "dashed", linewidth = 0.6) +
  facet_wrap(~ SUBJID, scales = "free_y") +
  labs(
    title = "Bayesian TGI Model Fit with FUP on Prime Data",
    y = "SLD",
    x = "Time (years)",
    color = "Type"
  ) +
