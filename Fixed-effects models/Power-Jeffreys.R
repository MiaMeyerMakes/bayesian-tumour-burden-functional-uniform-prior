...
# Jeffreys prior
log_prior_theta <- function(theta, x) {
  theta_prior = sqrt(sum((x^2)/((x+theta)^4)))
...
log_posterior <- function (theta0=0.2, theta1=0.6, theta2, sig2, x, y) {
  log_post = log_likelihood(theta0, theta1, theta2, sig2, x, y) + log_prior_theta(theta2, x) + log_prior_sigma2(sig2)
  return(log_post)
}
...
power_metropolis_hastings <- function(dataset, num_iterations, burn_in, sigma_theta, sigma_sigma, initial_theta, initial_sigma2) {
  ...
  for (setnr in 2:ncol(dataset))
  {
    ...
    for (iteration in 1:num_iterations) {
      ...
      sigma2_star <-  rlnorm(0, 0.2)
      
      # Step 5: Compute acceptance ratio for sigma2
      alpha_sigma_num <- log_posterior(theta2=theta_current, sig2=sigma2_star, x=x, y=y) + log(dlnorm(sigma2_star,0, 0.2))
      alpha_sigma_denom <- log_posterior(theta2=theta_current, sig2=sigma2_current, x=x, y=y)  + log(dlnorm(sigma2_current,0, 0.2))
...
sigma_theta <- 0.08      # Step size for theta
...
power_jef_statistics_table = rbind(power_stats15, power_stats50, power_stats100)
row.names(power_jef_statistics_table) = c("n=15", "n=50", "n=100")
colnames(power_jef_statistics_table) = c("CP_90", "CP_95", "ILE90", "ILE95", "MAE_mean", "MAE_median")
power_jef_statistics_table
